...
tr_data<-transformedData[,1];
ts_data<-test_transformed[,1];
...
colnames(tr_data)[2] <- "target";
colnames(ts_data)[2] <- "target";
...
pt1<-ptt[ptt$target==1,]
pt2<-ptt[ptt$target==2,]
a<-pt1[,1];
b<-pt1[,2];
c<-pt1[,3];
d<-pt2[,1];
...
x <- list("Class1"=a, "Class2"=d)
stripchart(x,
           main="Projection of data on first Component",
           xlab="Component 1",
           ylab="Classes",
           method="jitter",
           col=c("orange","red"),
...
##plot discriminating plane 
x=c(a,d);
x<-cbind(rep(1,length(a)),x);
plot(x[,2],x[,1],col="red",xlab="Component 1",ylab="intercept",);
abline(a = fit$coefficients[2], b = fit$coefficients[1],col="Blue");
print(fit$coefficients);
